    ...
    <tr>
        ...
        <td align="right" class="w200">
            <select class="w200" onchange="window.open(this.value,'_self');" name="cat_id" id="cat_id">
                <option value="<?=site_url('daugia/tintuc/listcontent/0/?option=true'.$sections_url.$key_url)?>">--Chọn chủ đề--</option>
                <?php foreach($listcategory as $cat):?>
                <option value="<?=site_url('daugia/tintuc/listcontent/0/?option=true'.$sections_url.'&cat_id='.$cat->cat_id.$key_url)?>" <?php echo ($cat_id == $cat->cat_id)?'selected="selected"':'';?>><?php echo $cat->cat_title?></option>
...
<?
    $url = 'daugia/tintuc/listcontent/0/?option=true'.$sections_url.$cat_url;
...
<?php echo form_open('daugia/tintuc/dels',  array('id' => 'admindata'));?> 
<table class="admindata">
    <thead>
        ...
        <tr>
            ...
            <th class="id"><?php echo vnit_order('daugia/tintuc/listcontent/0/id/asc','ID')?></th>
            <th><?php echo vnit_order('daugia/tintuc/listcontent/0/title/asc','Tiêu đề')?></th>
            <th><?php echo vnit_order('daugia/tintuc/listcontent/0/catid/asc','Chủ đề')?></th>
    ...
    <tr class="row<?=$k?>">
        ...
        <td align="center">
            <?php echo icon_edit('daugia/tintuc/edit/'.$rs->id)?>
