...
class donhang_model extends CI_Model{
    ...
    function get_all_donhang($num, $offset, $tinhtrang, $field,$order){
        ...
        $sql = "
            SELECT 
                product_cheap_list.*, product_cheap_cart.*
            FROM 
                product_cheap_list, product_cheap_cart
            WHERE
                product_cheap_list.id = product_cheap_cart.cheap_id
        ";
        if($tinhtrang == 0){
            $sql .=" AND moinhat = 1";
        }else{
    ...
    function get_total_donhang(){
        ...
        return $this->db->get('product_cheap_cart')->num_rows();
    }
    ...
    function get_chitiet($cart_id){
        $this->db->select('product_cheap_list.*, product_cheap_cart.*');
        $this->db->join('product_cheap_list','product_cheap_list.id = product_cheap_cart.cheap_id');
        $this->db->where('product_cheap_cart.cart_id',$cart_id);
        return $this->db->get('product_cheap_cart')->row();
    }
    
    function get_sum_giamgia($cart_id){
        $this->db->select_sum('price');
        $this->db->where('cart_id',$cart_id);
        return $this->db->get('product_cheap_discount')->row()->price;
        
    }
    
    function get_all_giamgia($cart_id){
        $this->db->where('cart_id',$cart_id);
        return $this->db->get('product_cheap_discount')->result();
