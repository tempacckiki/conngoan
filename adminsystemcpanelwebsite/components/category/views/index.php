...
<table class="admindata">
    <thead>
        <tr>
            ...
            <th class="head" colspan="3" align="right">
                <select name="sections_id" onchange="window.open(this.value,'_self');">
                    <option value="<?php echo site_url(uri_string())?>"> --Chọn chủ đề--</option>
                    <?php foreach($listsections as $sec):?>
                    <option value="<?=site_url(uri_string().'/?option=true&sections_id='.$sec->sections_id)?>" <?php echo ($sections_id == $sec->sections_id) ? 'selected="selected"':''?>><?php echo $sec->sections_title?></option>
                    <?php endforeach;?>
                </select>            
            </th>
        </tr>
        <tr>
            ...
            <th style="width: 150px;"><?php echo vnit_order('category/listcategory/0/section/desc'.$sections_url,'Chủ đề')?></th>
    ...
    <tr class="row<?=$k?>">
        ...
        <td><?=$rs->cat_id?></td>
        <td><?=$rs->cat_title?></td>
        <td><?=$this->vdb->find_by_id('sections',array('sections_id'=>$rs->section))->sections_title?></td>
        <td align="center">
        ...
        <td class="copy" id="<?=$rs->cat_id?>" align="center">
            ...
            <input class="w200" id="link_<?=$rs->cat_id?>" type="hidden" value="content/<?php echo $this->vdb->find_by_id('sections',array('sections_id'=>$rs->section))->sections_alias.'-'.$rs->section?>/<?php echo $rs->cat_alias?>-<?php echo $rs->cat_id?>">
        </td>         
        <td align="center">
            ...
            <?php echo icon_del('category/del/'.$rs->cat_id.'/'.(int)$this->uri->segment(4))?>        
        </td>
    </tr>       
    <?php
    $k=1-$k;
