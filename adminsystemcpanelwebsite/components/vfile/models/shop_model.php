...
***************************/
  class shop_model extends CI_Model{
      ...
      function save_product_add(){
          ...
          // EN
          $productname_en       = $this->input->post('productname_en');
          $productnameurl_en    = vnit_change_title($productname_en);
          $mieuta_en            = $this->input->post('mieuta_en');
          $baiviet_en           = $this->input->post('baiviet_en');
          ...
          // Khoi tao du lieu EN;
          $endata['barcode']                    = $barcode;
          $endata['productname']                = $productname_en;
          $endata['producturl']                 = $productnameurl_en;
          $endata['catid']                      = $catid;
          $endata['manufactureid']              = $manufactureid;
          $endata['sphot']                      = $sphot;
          $endata['spmoi']                      = $spmoi;
          $endata['spkhuyenmai']                = $spkhuyenmai;
          $endata['baohanh']                    = $baohanh;

          $endata['phukien']                    = $phukien;
          $endata['soluong']                    = $soluong;
          $endata['mieuta']                     = $mieuta_en;
          $endata['baiviet']                    = $baiviet_en;
          $endata['video']                      = $video;

          if($productid = $this->vdb->update('shop_product',$vidata)){
                //$endata['productid'] = $productid;
                //$this->vdb->update('shop_product_en',$endata);
                ...
                for($i = 0; $i < sizeof($ar_img); $i++){
                    if($ar_img[$i]){
                        ...
                        vnit_resize_image($folder_tmpl.$ar_img[$i],ROOT.'data/img_product/500/'.$img_name_new,500,500);
                        vnit_resize_image($folder_tmpl.$ar_img[$i],ROOT.'data/img_product/300/'.$img_name_new,300,300);
                        vnit_resize_image($folder_tmpl.$ar_img[$i],ROOT.'data/img_product/200/'.$img_name_new,200,200);
                        vnit_resize_image($folder_tmpl.$ar_img[$i],ROOT.'data/img_product/80/'.$img_name_new,80,80);
                        vnit_resize_image($folder_tmpl.$ar_img[$i],ROOT.'data/img_product/40/'.$img_name_new,40,40);
                ...
                $new_dir ='data/img_rotare/'.$productid.'/';
      ...
      function save_product_edit(){
          ...
          if($this->vdb->update('shop_product',$vidata,array('productid'=>$productid))){
                //$this->vdb->update('shop_product_en',$endata,array('productid'=>$productid));
      ...
      function delete_product($productid){
          ...
          foreach($list as $rs):
            unlink(ROOT.'data/img_product/40/'.$rs->imagepath);
            unlink(ROOT.'data/img_product/80/'.$rs->imagepath);
            unlink(ROOT.'data/img_product/200/'.$rs->imagepath);
            unlink(ROOT.'data/img_product/300/'.$rs->imagepath);
            unlink(ROOT.'data/img_product/500/'.$rs->imagepath);
          endforeach;
          $this->db->where('productid',$productid);
