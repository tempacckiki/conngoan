<?php
  class sitemap_news extends CI_Controller{
          ...
          $str .= "<urlset
           ...
           if(count($listCat)>0){  
           		...
           		foreach ($listCat as $val): 
           			...
           			$linkCat 		= base_url_site().'tin-tuc/'.$val->caturl.'-'.$val->catid.'.html';            			
           			//create subcat
           			...
           			if(count($listProduct) >0){
           				foreach ($listProduct as $valP):
           					$linkP 		= base_url_site().'tin-tuc/'.$valP->caturl.'/'.$valP->title_alias.'-'.$valP->newsid.'.html';            					           			
		           			$str .= "\n<url>\n";
		           			$str .= "<loc>".$linkP."</loc>\n";        
		           			$str .= "<changefreq>".$changefreq."</changefreq>\n";        
		           			$str .= "<priority>".$priority."</priority>\n"; 
           			...
           			if (count($listsub)>0){
	           			...
	           			foreach ($listsub as $val2):
	           				...
	           				$linkCat2 		= base_url_site().'tin-tuc/'.$val2->caturl.'-'.$val2->catid.'.html';
	           				$str .= "\n<url>\n";   
		           			...
		           			if(count($listProduct2) >0){
		           				foreach ($listProduct2 as $valP2):		           					
		           					$linkP2 		= base_url_site().'tin-tuc/'.$valP2->caturl.'/'.$valP2->title_alias.'-'.$valP2->newsid.'.html';
				           			$str .= "\n<url>\n";
				           			$str .= "<loc>".$linkP2."</loc>\n";        
				           			$str .= "<changefreq>".$changefreq."</changefreq>\n";        
				           			$str .= "<priority>".$priority."</priority>\n"; 
		           			...
		           			if(count($listsub3)>0){
			           			...
			           			foreach ($listsub3 as $val3):
			           				$linkCat3 		= base_url_site().'tin-tuc/'.$val3->caturl.'-'.$val3->catid.'.html'; 
			           				//sub 4
			           				$str .= "\n<url>\n"; 
				           			...
				           			if(count($listProduct3) >0){
				           				foreach ($listProduct3 as $valP3):				           					         					           		
				           				    $linkP3 		= base_url_site().'tin-tuc/'.$valP3->caturl.'/'.$valP3->title_alias.'-'.$valP3->newsid.'.html';
				           					$str .= "\n<url>\n";
						           			$str .= "<loc>".$linkP3."</loc>\n";        
						           			$str .= "<changefreq>".$changefreq."</changefreq>\n";        
						           			$str .= "<priority>".$priority."</priority>\n"; 
           ...
           $str .= "\n</urlset>\n";        
          write_file(ROOT.'sitemapnews_'.$idCat.'.xml', $str);
      }
  }
