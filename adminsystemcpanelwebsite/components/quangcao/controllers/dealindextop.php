<?php
class dealindextop extends CI_Controller{
    ...
    function ds(){
        ...
        $data['add'] = 'quangcao/dealindextop/add|'.icon_add('quangcao/dealindextop/add');
        $data['delete'] = icon_dels('quangcao/dealindex/dels');
        $data['list'] = $this->vdb->find_by_list('dealindex',array('position'=>2),array('ordering'=>'asc'));
        
        //*** load templates **********************
        $this->_templates['page'] = 'dealindextop/index';
    ...
    function add(){
        ...
        $data['save'] = true;
        $data['apply'] = true;
        $data['cancel'] = 'quangcao/dealindextop/ds';
        ...
        if($this->form_validation->run() === FALSE){
        	...
        	$img =  $this->input->post('img');
          	if(!empty($img)){
          		...
          		$imgRoot    = ROOT.'alobuy0862779988/adv/dealindextop/full_images/'.$img;
          		$imgThumb   = ROOT.'alobuy0862779988/adv/dealindextop/thumb/'.$img;
            ...
            if($id = $this->vdb->update('dealindex',$vdata)){
               /// write_log(78,255,'Thêm quảng cáo deal index: '.$vdata['name']);
                ...
                if($option == 'save'){
                   $url = 'quangcao/dealindextop/ds/';
                }else{
                    $url = 'quangcao/dealindextop/edit/'.$id;
                }
                redirect($url);
        ...
        //***load templates *******************
        $this->_templates['page'] = 'dealindextop/add';
        $this->templates->load($this->_templates['page'],$data);
    }
    ...
    function edit(){
        ...
        $data['message'] = $this->pre_message;
        //** load templates *********************
        $this->_templates['page'] = 'dealindextop/edit';
        $this->templates->load($this->_templates['page'],$data);
    }
    ...
    function dels(){
        ...
        redirect('quangcao/dealindextop/ds/'.$page);
    }
    //*************************************
    function write_quangcao(){
        $list = $this->vdb->find_by_list('dealindex',array('published'=>1,'position'=>2),array('ordering'=>'asc'));
        $total = count($list);
        $str = "<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');\n/**\n* File config_dealindex language ".vnit_lang().".\n* Date: ".date('d/m/y H:i:s').".\n**/";
        $str .= "\n\$config['advDeal_totalTop'] = $total;\n"; 
        $i = 1;
        foreach($list as $rs):
            $name 		= $rs->name;
            ...
            $link 		= $rs->link;
            $img 		= $rs->images;
            $str .= "\n\$config['advDealTop_name_$i'] = '$name';";
            $str .= "\n\$config['advDealTop_decription_$i'] = '$decription';";
            $str .= "\n\$config['advDealTop_price_$i'] = '$price';";
            $str .= "\n\$config['advDealTop_priceOld_$i'] = '$priceOld';";
            $str .= "\n\$config['advDealTop_link_$i'] = '$link';";
            $str .= "\n\$config['advDealTop_img_$i'] = '$img';\n";
        $i ++;
        endforeach;
        $str .= "\n\n/* End of file config_dealindex*/";        
        write_file(ROOT_ADMIN.'config/config_dealindextop.php', $str);
    }
    ...
    function uploader(){
    	...
    	$dir 		= ROOT.'alobuy0862779988/adv/dealindextop/full_images/';
