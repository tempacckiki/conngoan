...
class danhmuc extends CI_Controller{
    ...
    function index(){
        redirect('sangiare/danhmuc/listcategory');
    }
    
    function listcategory(){
        $data['title'] = 'Quản lý Chủ đề';
        $data['add'] = 'sangiare/danhmuc/add';
        $data['delete'] = true;        
        $data['sections_id'] = (int)$this->input->get('sections_id');
        $data['listsections'] = $this->vdb->find_by_list('category',array('site'=>2));
    ...
    function add(){
        ...
        $data['save'] = true;
        $data['apply'] = true;
        $data['cancel'] = 'sangiare/danhmuc/listcategory';
        ...
        if($this->form_validation->run() === FALSE){
            ...
            $cat['lang'] = vnit_lang();
            $cat['site'] = 2;
            if($cat_id = $this->vdb->update('category',$cat)){
                ...
                $cat_en['site'] = 2;
                $this->vdb->update('category_en',$cat_en);
                $this->session->set_flashdata('message','Lưu thành công');
                $option =  $this->input->post('option');
                if($option == 'save'){
                   $url = 'sangiare/danhmuc/listcategory';
                }else{
                    $url = 'sangiare/danhmuc/edit/'.$id;
                }
                redirect($url);
            }
        }
    ...
    function edit(){
        ...
        $data['apply'] = true;
        $data['cancel'] = 'sangiare/danhmuc/listcategory';        
        $data['rs'] = $this->vdb->find_by_id('category',array('cat_id'=>$this->uri->segment(4)));
        $data['rs_en'] = $this->vdb->find_by_id('category_en',array('cat_id'=>$this->uri->segment(4)));
        $data['list'] = $this->vdb->find_by_list('category',array('site'=>2,'parent_id'=>0));
        // Form validation
        $this->form_validation->set_rules('cat[cat_title]','Tiêu đề - vi','required');
        $this->form_validation->set_rules('cat_en[cat_title]','Tiêu đề - en','required');
        ...
        if($this->form_validation->run() === FALSE){
            ...
            if($this->vdb->update('category',$cat,array('cat_id'=>$cat_id))){
                ...
                $cat_en['published'] = $cat['published'];
                $cat_en['site'] = 2;
                $this->vdb->update('category_en',$cat_en,array('cat_id'=>$cat_id));
                // Update content VI;
                $con['cat_alias'] = $cat['cat_alias'];
                $this->vdb->update('content',$con,array('catid'=>$cat_id));
                // Update content en
                $con['cat_alias'] = $cat_en['cat_alias'];
                $this->vdb->update('content_en',$con,array('catid'=>$cat_id)); 
    ...
    function dels(){
            ...
            for($i = 0; $i < sizeof($ar_id); $i ++) {
                if ($ar_id[$i]){
                  if($this->vdb->find_by_max('content','',array('catid'=>$ar_id[$i])) > 0){
                      $this->session->set_flashdata('error','Chủ đề: <b>'.$this->vdb->find_by_id('category',array('cat_id'=>$ar_id[$i]))->cat_title.'</b> vẫn còn bài viết. Vui lòng xóa bài viết');
                  }else{                        
                        if($this->vdb->delete('category', array('cat_id'=>$ar_id[$i]))){
                            $this->vdb->delete('category_en', array('cat_id'=>$ar_id[$i]));
        ...
        redirect('sangiare/danhmuc/listcategory/'.$page);
    }      
}