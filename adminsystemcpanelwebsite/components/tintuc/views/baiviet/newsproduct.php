    ...
    <tr>
        ...
        <td align="right" class="w200">
            <select class="w200" onchange="window.open(this.value,'_self');" name="cat_id" id="cat_id">
                <option value="<?=site_url('tintuc/photonews/readPhoto/0/?option=true'.$key_url)?>">--Tất cả danh mục--</option>
                <?php foreach($listcat as $cat):?>
                <option value="<?=site_url('tintuc/photonews/readPhoto/0/?option=true&cat_id='.$cat->catid.$key_url)?>" <?php echo ($cat_id == $cat->catid)?'selected="selected"':'';?>><?php echo $cat->catname?></option>
                <?php endforeach;?>
            </select>
        </td>
    </tr>
...
<?
    $url = 'tintuc/photonews/readPhoto/0/?option=true'.$cat_url;
...
<?php echo form_open('tintuc/photonews/delsPhoto',  array('id' => 'admindata'));?> 
<table class="admindata">
    <thead>
        ...
        <tr>
            ...
            <th class="id"><?php echo vnit_order('tintuc/photonews/readPhoto/0/newsid/asc','ID')?></th>
            <th><?php echo vnit_order('tintuc/photonews/readPhoto/0/title/asc','Tiêu đề')?></th>
            <th style="width: 150px;"><?php echo vnit_order('tintuc/photonews/readPhoto/0/catid/asc','Danh mục')?></th>
    ...
    <tr class="row<?=$k?>">
        ...
        <td>
        	<input type="text" id="thutu_<?=$rs->newsid;?>" value="<?=$rs->order_detail;?>">
        </td>
        ...
        <td align="center">
            ...
            <?php echo icon_del('tintuc/photonews/delPhoto/'.$rs->newsid.'/'.(int)$this->uri->segment(4))?>        
        </td>
    </tr>       
    <?php
    $k=1-$k;
...
<script type="text/javascript">
    function save_edit(id){                          	     
        thutu 		= $("#thutu_"+id).val();                     
        $.post(base_url+"tintuc/newsproduct/saveAjaxPhoto",
        {  
            'order_detail':thutu,           
            'newsid':id
        },function(data){           
