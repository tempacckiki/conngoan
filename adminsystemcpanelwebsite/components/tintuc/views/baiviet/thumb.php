    ...
    <tr>
        ...
        <td align="right" class="w200">
            <select class="w200" onchange="window.open(this.value,'_self');" name="cat_id" id="cat_id">
                <option value="<?=site_url('tintuc/readThumb/0/?option=true'.$key_url)?>">--Tất cả danh mục--</option>
                <?php foreach($listcat as $cat):?>
                <option value="<?=site_url('tintuc/readThumb/0/?option=true&cat_id='.$cat->catid.$key_url)?>" <?php echo ($cat_id == $cat->catid)?'selected="selected"':'';?>><?php echo $cat->catname?></option>
                <?php endforeach;?>
            </select>
        </td>
    </tr>
...
<?
    $url = 'tintuc/readThumb/0/?option=true'.$cat_url;
...
<?php echo form_open('tintuc/delsThumb',  array('id' => 'admindata'));?> 
<table class="admindata">
    ...
    <tr class="row<?=$k?>">
        ...
        <td align="center">
            ...
            <?php echo icon_del('tintuc/delThumb/'.$rs->newsid.'/'.(int)$this->uri->segment(4))?>        
        </td>
    </tr>       
    <?php
    $k=1-$k;
...
<script type="text/javascript">
    function save_edit(id){                         	      
        thutu 		= $("#thutu_"+id).val();                     
        $.post(base_url+"tintuc/saveAjaxthumb",
        {  
            'order_thumb':thutu,           
            'newsid':id
        },function(data){
