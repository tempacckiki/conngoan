...
function get_feature_list($feature_id){
    $CI = get_instance();
    $CI->db->select('shop_feature_variants.*, shop_feature_variant_descriptions.*');
    $CI->db->join('shop_feature_variant_descriptions','shop_feature_variant_descriptions.variant_id = shop_feature_variants.variant_id');
    $CI->db->where('shop_feature_variants.feature_id',$feature_id);
    $CI->db->order_by('shop_feature_variant_descriptions.variant','asc');
    return $CI->db->get('shop_feature_variants')->result();
}
...
function remove_feature($feature_id,$value){
    $url = get_url();
    $temp = explode('&',$url);
    if(count($temp) > 1){
        $ar_find = array("$feature_id=$value&","$feature_id=$value");
        $ar_rep = array('','');
        return trim(str_replace($ar_find,$ar_rep,$url),'&');
    }else{
        $ar_find = "?$feature_id=$value";
        return str_replace($ar_find,'',$url);
    }
    
}

function vtime($totalSeconds){
	if ($totalSeconds <= 0)
		return "00 : 00 : 00";

	$second = round($totalSeconds%60,0);
	$minute = round(($totalSeconds / 60) % 60,0);
	$hour = round(($totalSeconds / 3600)%24,0);
	$day = explode('.',($totalSeconds / 3600)/24);
	...
	return $_day." ngày, ".$_hour." giờ, ".$_minute." phút, ".$_second." s";

}

function get_city(){
    return get_cookie('fyi_choice_city');
}

function type_img($img){
...
function _check_mabaove($code){ 
    ...
    $binds = array($code, $CI->input->ip_address(), $expiration);
    $query = $CI->db->query($sql, $binds);
    $row = $query->row();

    if ( $row -> count > 0 ){
        $CI->vdb->delete('captcha',array('session_id'=>$CI->session->userdata('session_id')));
        return true;
    }else{
        return false;
    }
